...
- [x] 2026-09-12 3S speaker stutter fixed (72 → 0 underruns): root cause was the main loop — WebSocketsClient handles one frame per `loop()` and the 60 ms frame push capped audio delivery at exactly real time. Now: frame push on its own task (shadow copy, main loop ~4 ms), `ws.loop()` drained 8× per loop, 2 s ring, brain lead scales with `capabilities.audio_buffer_s` (1.2 s), 700 ms prime before output (Piper synthesises the first clause at ~1× real time). fw 0.7.0
- [ ] 3S: re-layout the settings/status screens for 480×320 (currently a centred 320×240 layer); frame push ~80 ms on core 0 (~12 fps face) → dirty-row transposition, try core 1, QSPI DMA
...
- [x] Settings menu v2 (2026-09-11, fw 0.6.1): Connectivity (Wi-Fi with Change Wi-Fi; Network with IP/GW/DNS/MAC + speed test: DNS, HTTP, 5 brain pings, 256 KB download, 128 KB upload), Sound, Power, Pixel (Portal+Unpair, Update, System+Event log, Reset), Diagnostics (Pipeline, Event log), Face; breadcrumbs and badges
- [x] OTA hardening (2026-09-11): the manifest now arrives over the brain WebSocket (`ota_check` → `ota_manifest`) and the image + speed test use plain HTTP (Caddy keeps `/firmware/*` and `/api/speedtest/*` on :80; SHA-256 from the manifest guards the image). Reason: with the speaker firmware the largest free heap block is ~16 KB while the TLS link is up (~43 KB with it closed) — a second TLS session no longer fits, so HTTPS manifest checks had been failing silently. Verified 0.5.99 → 0.6.1 OTA.
- [ ] **Bandwidth budget (measured 2026-09-11, home Wi-Fi at -30 dBm → Hetzner Germany):** ~310 kbit/s down, ~170 kbit/s up per TCP connection — the ESP32's 5.7 KB lwIP window × ~100 ms RTT, not the Wi-Fi. 16 kHz PCM speech is 256 kbit/s: downlink barely fits (hence brain-side pacing), uplink mic audio will NOT fit as raw PCM → compress (ADPCM/µ-law 4:1 or Opus) or drop to 8 kHz before the mic work; a US-East brain would roughly double both
