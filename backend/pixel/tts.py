...
def synthesize(text: str):
    ...
    if hasattr(voice, "synthesize_stream_raw"):            # piper-tts 1.2.x
        for raw in voice.synthesize_stream_raw(text):
            yield _resample(np.frombuffer(raw, dtype=np.int16), src_rate, C.SAMPLE_RATE).tobytes()
    else:                                                    # piper-tts >= 1.3: AudioChunk objects
        for chunk in voice.synthesize(text):
            yield _resample(np.frombuffer(chunk.audio_int16_bytes, dtype=np.int16), chunk.sample_rate, C.SAMPLE_RATE).tobytes()
