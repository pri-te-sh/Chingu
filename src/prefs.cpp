...
void load() {
  ...
  volume = p.getUChar("vol", volume); batteryTalk = p.getBool("btalk", true);
...
void save() {
  p.begin("pixel", false);
  p.putString("name", name); p.putUInt("eye", eyeRGB); p.putUShort("sleep", autoSleepS); p.putUChar("vol", volume); p.putBool("btalk", batteryTalk); p.putString("moods", moods);
